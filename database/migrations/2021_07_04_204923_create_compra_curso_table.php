...
class CreateCompraCursoTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('compra_curso', function (Blueprint $table) {
            ...
            $table->boolean('tr_com_cur_vigencia');
            $table->foreign('tr_com_cur_id_com_fk')->references('tr_com_id')->on('compra');
            $table->foreign('tr_com_cur_id_cur_fk')->references('tr_cur_id')->on('cursos');
        });
    }
